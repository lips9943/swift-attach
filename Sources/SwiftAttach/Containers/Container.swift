...
public struct Container {
    ...
    public func register<T>(impl value: T, scope: Scope = .transient) {
    ...
    ///   - value: 등록할 인스턴스
    ///   - scope: 인스턴스의 생명주기 (기본값: `.transient`)
    public func register<T, P>(protocol: P.Type, impl value: T,  scope: Scope = .transient) {
...
extension Container {
    ...
    public func register<T, P>(customKey: String, protocol: P.Type, impl value: T, scope: Scope = .transient) {
        let key = makeKey(customKey, protocols: `protocol`, scope: scope)
        guard !hasKey(key: key) else { return }
        _registerByScope(key: key, impl: value, scope: scope)
    }
}
...
extension Container {
    ...
    private func makeKey(_ type: String, protocols: Any.Type?, scope: Scope) -> String {
    ...
    private func _registerByScope(key: String, impl value: Any, scope: Scope) {
        container.withLock {
            let value: () -> Any = { value }
